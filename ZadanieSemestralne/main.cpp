...
#include "obslugaGry.cpp"

#define MAIN_CPP

/*	Wypisuje inforamacje dla uzytkownika.
*/

void info(void) {
	cout << "Program realizuje gre w Wisielca.\n";
	cout << "Autor: Lukasz Lipski\n\n";
	cout << "Instrukcja:\n";
	cout << "Celem gry jest odgadniecie hasla litera po literze lub podajac cale haslo.\n";
	cout << "Haslo skalada sie tylko z liter alfabetu lacinskiego.\n";
	cout << "Litery male i duze nie sa rozroznialne przy zgadywaniu.\n";
	cout << "Na graczy przypada 9 niepowodzen. Po uzyskaniu 10 gra jest konczona i gracze przegrywaja.\n";
	cout << "Wpisanie podanej wczesniej litery skutkuje niepowodzeniem.\n";
}
...
int main() {
	info();
	srand(time(NULL));
	Obsluga_gry gra; //klasa do obslugi gry
	cout << "\nCzy chcesz wpisac swoje haslo (T - tak, N - nie)? ";
	gra.sprawdz_czy_pobrac_haslo();
	printf("Podaj ilu graczy ma byc w grze? (maksymalnie %d): ", MAX_GRACZY);
	gra.pobierz_ilosc_graczy();
	gra.stworz_graczy();
	cout << "\nGra rozpoczyna sie.\n";
	do {
		gra.wypisz_informacje();
		printf("\nKolej gracza %d.\n", gra.aktualny->get_nr_gracza());
		if (gra.aktualny->get_tryb() == TRYB_UZYTKOWNIKA) {
			printf("Czy chcesz podac litere czy odgadnac haslo (%c - litera, %c - haslo): ", LITERA, HASLO);
			if (gra.wczytaj_odgadywanie() == LITERA) {
				gra.wczytaj_litere();
				gra.spr_litere();
			}
			else {
				gra.wczytaj_spr_haslo();
				gra.sprawdz_haslo();
			}
		}
		else {
		...
		gra.aktualny = gra.aktualny->nast;
		if (gra.aktualny == NULL) {
			gra.aktualny = gra.pierwszy;
			gra.set_liczba_kolejek(gra.get_liczba_kolejek()+1);
		}
	} while (!gra.spr_czy_koniec_gry());
	printf("\nHaslo: %s\n", gra.get_haslo());
	printf("Kategoria: %s\n", gra.get_kategoria());
	cout << "Statystyki:\n";
	gra.wypisz_statystyki();
	cout << "\nKoniec programu.\n";
	return 0;
}